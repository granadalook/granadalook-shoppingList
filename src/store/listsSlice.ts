...
export const createListAsync = createAsyncThunk(
  ...
  async (
    payload: { id: string | null; name: string; owner: string | null; image?: string },
    { rejectWithValue }
  ) => {
    try {
...
export const shareListAsync = createAsyncThunk(
  ...
  async (
    payload: { listId: string; user: string; owner?: string | null; name?: string },
    { rejectWithValue }
  ) => {
    try {
      await axios.put(`${BASE_URL}/entidad/${payload.listId}`, {
        creadoPor: payload.user,
        // Aquí podrías agregar más datos si el backend lo permite
      });
      return payload;
    } catch (err: any) {
      return rejectWithValue(err.message);
...
export const deleteItemAsync = createAsyncThunk(
  ...
  async (
    { listId, item }: { listId: string; item: string },
    { rejectWithValue }
  ) => {
    try {
...
const listsSlice = createSlice({
  ...
  extraReducers: builder => {
    ...
    builder.addCase(shareListAsync.fulfilled, state => {
      state.loading = false;
    });
